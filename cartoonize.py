...
class Cartoonizer():
    ...
    def draw_points(self, img, landmark, n = 0):
        count = 0
        for thing in landmark:
            x, y = thing[0], thing[1]
            center = (int(x), int(y))
            cv2.circle(img, center, 10, (255, 255, 255), -1)
            count += 1
        cv2.imwrite("face_with_landmarks.jpg", img)
        # cv2.imwrite('landmark_images_2/img' + str(n) + '.png', img)
        print("Face with landmarks generated.")

    def cartoonize(self, img, n = 0):
        # img: RGB input
        ...
        eye_right, eye_left, nose = None, None, None
        for landmark in landmarks:

            # self.draw_points(img, landmark, n)

            # drawing the eyes
            eye_right = [landmark[39], landmark[38], landmark[37], landmark[36], landmark[41], landmark[40]]
            eye_left = [landmark[42], landmark[43], landmark[44], landmark[45], landmark[46], landmark[47]]
            nose = [landmark[i] for i in range(27, 36)]

            # draw_polygon(eye_right, img)
            # draw_polygon(eye_left, img)
            # cv2.imwrite("face_with_polygon.jpg", img)
        ...
        return res, eye_right, eye_left, nose, f5p[0], f5p[1], f5p[2]
